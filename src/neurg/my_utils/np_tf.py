...
def T2xyzrpy(T, degrees=False):
    return np.r_[T[:3, 3].copy(), R2euler(T[:3, :3], degrees=degrees)]


def xyzrpy2T(xyzrpy, degrees=False):
    t = xyzrpy[:3]
...
def T34dot(T1, T2):
    return np.c_[T1[:3, :3].dot(T2[:3, :3]), T1[:3, :3].dot(T2[:3, 3]) + T1[:3, 3]]
...
def T2xyzwxyz7(T):
    """T -> xyz, quat(wxyz)"""
    xyzw = sR.from_matrix(T[:3, :3]).as_quat()
    return np.r_[T[:3, 3], xyzw[3], xyzw[0:3]]
