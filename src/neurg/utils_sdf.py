...
def b_inv_mat44_to_mat34(T_w2links: torch.Tensor):
    """
    T_w2links: x, n_ch, 4, 4; x,
    """
    shape = T_w2links.shape
    T_w2links = T_w2links.view(-1, shape[-2], shape[-1])
    RT = T_w2links[:, :3, :3].transpose(-1, -2)
    return torch.concatenate([RT, -(RT @ T_w2links[:, :3, [3]])], dim=-1).view(shape[0], shape[1], 3, 4)
...
class RobotConfig(torch.nn.Module):
    ...
    def __init__(self):
        ...
        T_link7toee = DH2mat34(DH_Params[0][i], DH_Params[1][i] + 0.065, DH_Params[2][i], torch.tensor(-torch.pi / 4))
